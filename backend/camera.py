...
class CameraHandler:
    ...
    def process_frame(self, frame):
        ...
        # 绘制姿态骨架，使用设置的线条宽度
        processed_frame = results.plot(line_width=self.evaluator.line_width if hasattr(self.evaluator, 'line_width') else 3)
        
        # 在图像上显示动作信息
        if evaluation_result["angle"] > 0:
            # 在图像上显示角度
            cv2.putText(processed_frame, f"角度: {evaluation_result['angle']:.1f}°", (10, 30), 
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            # 在图像上显示次数
            cv2.putText(processed_frame, f"次数: {evaluation_result['count']}", (10, 70), 
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            # 在图像上显示阶段
            cv2.putText(processed_frame, f"阶段: {evaluation_result['stage']}", (10, 110), 
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
