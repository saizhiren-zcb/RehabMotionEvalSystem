...
class RehabEvaluator:
    """
    康复动作评估器类
    用于评估康复动作，包括角度计算、动作计数和阶段判断
    直接使用YOLO模型进行姿态估计和动作评估
    """
        ...
        if results.keypoints is not None:
            if len(results.keypoints.data) > 0:
                ...
                if track_id not in self.states:
                    # 初始阶段判断：使用新的三阶段规则
                    initial_stage = "down" if angle <= action["down_angle"] else "up" if angle >= action["up_angle"] else "normal"
                ...
                # 判断角度变化方向
                prev_direction = state["angle_direction"]
                angle_direction = "stable"
